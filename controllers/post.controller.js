...
const uploadFiles = expressAsyncHandler(
  async (req, res, next, files, postId) => {
    try {
      if (Object.entries(files).length == 0) {
        res
          .status(StatusCodes.BAD_REQUEST)
          .json({ message: "Please select a file" });
        return;
      }
...
const createPost = expressAsyncHandler(async (req, res, next) => {
  ...
  if (!isMultipart) {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .json({ message: "Form must be of type 'multipart/form-data'. " });
  }
...
const getSingleUserPostCommentReplyAndLikes = expressAsyncHandler(
  async (req, res, next) => {
    try {
      const authId = req;
      const post = await DB.post.findAll({
...
const getAllUsersPostCommentsReplysAndLikes = expressAsyncHandler(
  async (req, res, next) => {
    try {
      const post = await DB.post.findAll({
        include: [
